...
export const useAuthStore = defineStore('auth', {
  ...
  actions: {
    ...
    async signup(userDto: UserDto): Promise<boolean> {
      this.error = null;
      try {
        const response = await axios.post<UserDto>('https://back1-production-237e.up.railway.app/auth/signup', userDto);
        return true;
      } catch (error: any) {
    ...
    async login(loginDto: LoginDto): Promise<boolean> {
      this.error = null;
      try {
        const response = await axios.post<LoginResponse>('https://back1-production-237e.up.railway.app/auth/login', loginDto);
