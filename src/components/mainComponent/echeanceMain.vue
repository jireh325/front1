...
export default defineComponent({
  ...
  setup() {
    ...
    onMounted(async() => {
      ...
      const statusToKeep = ['reassigned', 'late'];
    ...
    const getCardStatusClass = (status: string) => {
      if (status === 'reassigned') return 'reassigned-todo';
      if (status === 'late') return 'late-todo';
      return '';
    };

    const getIconStatusClass = (status: string) => {
      if (status === 'reassigned') return 'reassigned-icon';
      if (status === 'late') return 'late-icon';
      return '';
    };

    const getIconColor = (status: string) => {
      if (status === 'reassigned') return '#9e9e9e';
      if (status === 'late') return '#f44336';
      return '#5d6b8d';
    };

    const getChipColor = (status: string) => {
      if (status === 'reassigned') return 'grey';
      if (status === 'late') return 'error';
      return 'primary';
    };

    const getStatusIcon = (status: string) => {
      if (status === 'reassigned') return 'mdi:clipboard-alert';
      if (status === 'late') return 'mdi:history';
